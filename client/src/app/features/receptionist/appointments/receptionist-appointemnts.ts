...
export class ReceptionistAppointmentsComponent implements OnInit, OnDestroy {
  ...
  /** ✅ NEW: min datetime to block past dates */
  minDateTime!: string;
  ...
  ngOnInit(): void {
    ...
    this.subs.add(
      this.patientSearchCtrl.valueChanges.pipe(
        ...
        switchMap(term =>
          term ? this.http.searchReceptionistPatients(term) : this.http.getReceptionistPatients()
        )
    ...
    this.subs.add(
      this.doctorSearchCtrl.valueChanges.pipe(
        ...
        distinctUntilChanged(),
        switchMap(term =>
          term ? this.http.searchReceptionistDoctors(term) : this.http.getDoctors()
        )
    ...
    this.subs.add(
      this.notificationService.sysEvents$.subscribe(event => {
        if (event?.action?.startsWith('appointment')) {
          this.loadAppointments();
        }
      })
    );
